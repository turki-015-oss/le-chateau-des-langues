...
const files=fs.readdirSync(root).filter(name=>name.startsWith("b2-")&&name!=="b2-factory.ts").sort();
const older=fs.readdirSync(root).filter(name=>/^(?:a1|a2|b1)-.*\.ts$/.test(name)&&!name.endsWith("-factory.ts"));
...
const lessonPattern=/b2Lesson\(\{id:"([^"]+)",number:(\d+),titleFr:"([^"]+)",titleAr:"([^"]+)"/g;
...
if(files.length!==12)fail.push(`expected 12 B2 data files, got ${files.length}`);
for(const file of sources){
 const count=[...file.text.matchAll(lessonPattern)].length;
 if(count!==6)fail.push(`${file.name} must contain 6 lessons, got ${count}`);
}
if(lessons.length!==72)fail.push(`expected 72 lessons, got ${lessons.length}`);
const numbers=lessons.map(lesson=>lesson.number).sort((a,b)=>a-b);
if(numbers.some((number,index)=>number!==index+1))fail.push("lesson numbering is not exactly 1..72");
if(lessons.some(lesson=>!lesson.id.startsWith("b2-")))fail.push("lesson id without b2- prefix");
...
if(duplicateTitles.length)fail.push(`duplicate B2 lesson titles: ${[...new Set(duplicateTitles)].join(" | ")}`);
const repeatedOlderTitles=lessons.map(lesson=>lesson.titleFr).filter(title=>olderTitles.has(title));
if(repeatedOlderTitles.length)fail.push(`lesson titles already used in A1/A2/B1: ${[...new Set(repeatedOlderTitles)].join(" | ")}`);
if(examples.length!==216)fail.push(`expected 216 examples, got ${examples.length}`);
if(comparisonPairs.length!==72)fail.push(`expected 72 comparison pairs, got ${comparisonPairs.length}`);
const duplicateFrench=examples.map(example=>example.fr).filter((fr,index,all)=>all.indexOf(fr)!==index);
if(duplicateFrench.length)fail.push(`duplicate B2 examples: ${[...new Set(duplicateFrench)].join(" | ")}`);
const collisions=examples.map(example=>example.fr).filter(fr=>olderFrench.has(fr));
if(collisions.length)fail.push(`examples already used in A1/A2/B1: ${[...new Set(collisions)].join(" | ")}`);
