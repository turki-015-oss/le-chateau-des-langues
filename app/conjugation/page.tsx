...
const BATCH_FULL=new Set(["maigrir","réfléchir","remplir","obéir","punir","bâtir","rougir","blanchir","agir","servir"]);
...
function reviewedBatchExample(form:string,title:string,index:number,verb:string):[string,string]|undefined{
 if(!BATCH_FULL.has(verb))return undefined;
 const c=BATCH_CONTEXT[verb];
 const fr=cap(form)+(title.startsWith("Infinitif")||title.startsWith("Participe")||title.startsWith("Gérondif")?" ":" ")+c.fr+".";
 const subjects=["أنا","أنت","هو/هي","نحن","أنتم","هم/هن"];
 const time:Record<string,string>={"Présent":"في هذا السياق","Passé composé":"وقد حدث ذلك بالفعل","Imparfait":"وكان ذلك يحدث سابقًا","Plus-que-parfait":"وكان ذلك قد حدث من قبل","Passé simple":"وحدث ذلك في الماضي","Passé antérieur":"وكان ذلك قد اكتمل قبل حدث آخر","Futur simple":"وسيحدث ذلك لاحقًا","Futur antérieur":"وسيكون ذلك قد اكتمل لاحقًا","Passé":"وقد كان ذلك سيحدث","Imparfait du subjonctif":"في صيغة أدبية قديمة","Plus-que-parfait du subjonctif":"في صيغة أدبية مركبة"};
 const ar=(subjects[Math.min(index,5)]||"")+" "+c.ar+" "+(time[title]||"بحسب الصيغة المطلوبة")+".";
 return [fr,ar];
}
...
export default function Page(){
 ...
 return <main className="conj-page conj-pro" dir="rtl">
  ...
  <section className="conj-heading"><div><span>{group(v)}</span><h1 dir="ltr">{v}</h1><h2>{AR[v]||"فعل فرنسي"}</h2><div className="conj-verb-meta" dir="ltr">{BATCH_FULL.has(v)&&<b>v6.0 · lot complet 1/18</b>}<b>{regularity(v)}</b><b>{trait(v)}</b><b>auxiliaire: {a}</b><b>participe passé: {pp}</b></div></div><button onClick={()=>speak(v)}><Volume2/></button></section>
  <UsagePanel verb={v}/>
  <section className="conj-content">{tab==='Indicatif'&&<><h2>Indicatif</h2><div className="conj-grid-pro"><Block title="Présent" forms={rows(p,v)} verb={v}/><Block title="Passé composé" forms={rows(AP[a].map(x=>x+' '+pp),v)} verb={v}/><Block title="Imparfait" forms={rows(imp(v),v)} verb={v}/><Block title="Plus-que-parfait" forms={rows(AI[a].map(x=>x+' '+pp),v)} verb={v}/>{BATCH_FULL.has(v)?<><Block title="Passé simple" forms={rows(pastSimple(v),v)} verb={v}/><Block title="Passé antérieur" forms={rows(AS[a].map(x=>x+' '+pp),v)} verb={v}/></>:<><ReviewBlock title="Passé simple"/><ReviewBlock title="Passé antérieur"/></>}<Block title="Futur simple" forms={rows(fut(v),v)} verb={v}/><Block title="Futur antérieur" forms={rows(AF[a].map(x=>x+' '+pp),v)} verb={v}/></div></>}
  {tab==='Conditionnel'&&<><h2>Conditionnel</h2><div className="conj-grid-pro"><Block title="Présent" forms={rows(cond(v),v)} verb={v}/><Block title="Passé" forms={rows((a==="avoir"?["aurais","aurais","aurait","aurions","auriez","auraient"]:["serais","serais","serait","serions","seriez","seraient"]).map(x=>x+' '+pp),v)} verb={v}/></div></>}
  {tab==='Subjonctif'&&<><h2>Subjonctif</h2><div className="conj-grid-pro"><Block title="Présent" forms={sub(v).map((x,i)=>["que je","que tu","qu’il / elle","que nous","que vous","qu’ils / elles"][i]+' '+x)} verb={v}/><Block title="Passé composé" forms={(a==="avoir"?["que j’aie","que tu aies","qu’il ait","que nous ayons","que vous ayez","qu’ils aient"]:["que je sois","que tu sois","qu’il soit","que nous soyons","que vous soyez","qu’ils soient"]).map(x=>x+' '+pp)} verb={v}/>{BATCH_FULL.has(v)?<><Block title="Imparfait du subjonctif" forms={subjRows(subjImperfect(v))} verb={v}/><Block title="Plus-que-parfait du subjonctif" forms={subjRows(SI[a].map(x=>x+' '+pp))} verb={v}/></>:<><ReviewBlock title="Imparfait"/><ReviewBlock title="Plus-que-parfait"/></>}</div></>}
  {tab==='Impératif'&&<><h2>Impératif</h2><div className="conj-grid-pro"><Block title="Présent" forms={[p[1],p[3],p[4]]} verb={v}/><Block title="Passé" forms={auxImp(a).map(x=>x+" "+pp)} verb={v}/></div></>}
  ...
  {tab==='Participe'&&<><h2>Participe</h2><div className="conj-grid-pro"><Block title="Présent" forms={[p[3].replace(/ons$/,'ant')]} verb={v}/><Block title="Passé" forms={[pp,auxPart(a)+" "+pp]} verb={v}/></div></>}
  {tab==='Gérondif'&&<><h2>Gérondif</h2><div className="conj-grid-pro"><Block title="Présent" forms={["en "+p[3].replace(/ons$/,'ant')]} verb={v}/><Block title="Passé" forms={["en "+auxPart(a)+" "+pp]} verb={v}/></div></>}</section>
 </main>
}
