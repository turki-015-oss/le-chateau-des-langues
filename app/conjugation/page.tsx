...
const VERB_TRAITS:Record<string,string>={
 ...
 "passer":"transitif / intransitif", "servir":"transitif / intransitif", "se rendre":"pronominal", "se servir":"pronominal"
};
...
export default function Page(){
 ...
 return <main className="conj-page conj-pro" dir="rtl">
  ...
  <section className="conj-heading"><div><span>{group(v)}</span><h1 dir="ltr">{v}</h1><h2>{AR[v]||"فعل فرنسي"}</h2><div className="conj-verb-meta" dir="ltr"><b>{regularity(v)}</b><b>{trait(v)}</b><b>auxiliaire: {a}</b><b>participe passé: {pp}</b></div><p>صفحة احترافية مرتبة حسب الصيغ والأزمنة، مع تعريف نوع الفعل وصفته، ونطق وأمثلة سياقية مراجعة تدريجيًا.</p><div className="conj-review-progress">v5.1 · البنية الكاملة الجديدة · لا تُعرض أمثلة أو تصريفات غير مراجعة</div></div><button onClick={()=>speak(v)}><Volume2/></button></section>
  <UsagePanel verb={v}/>
  ...
  {tab==='Subjonctif'&&<><h2>Subjonctif</h2><div className="conj-grid-pro"><Block title="Présent" forms={sub(v).map((x,i)=>["que je","que tu","qu’il / elle","que nous","que vous","qu’ils / elles"][i]+' '+x)} verb={v}/><Block title="Passé composé" forms={["que j’aie","que tu aies","qu’il ait","que nous ayons","que vous ayez","qu’ils aient"].map(x=>x+' '+pp)} verb={v}/><ReviewBlock title="Imparfait"/><ReviewBlock title="Plus-que-parfait"/></div></>}
