...
async function main() {
  ...
  const neutral = i => {
    ...
    return Math.min(r,g,b)>Number(process.argv[5] || 215) && Math.max(r,g,b)-Math.min(r,g,b)<13;
  };
  ...
  await image.clone().webp({quality:95,alphaQuality:100}).toFile(process.argv[4] || 'public/kingdom-portal-assets/university-campus-front-v3.webp');
