...
export default function GrammarLevelClient({level}:{level:string}){
 ...
 const filtered=useMemo(()=>{const q=query.trim().toLocaleLowerCase("fr");return q?lessons.filter(l=>[l.titleFr,l.titleAr,l.category,l.summary,l.rule,l.formula].join(" ").toLocaleLowerCase("fr").includes(q)):lessons},[lessons,query]);
 ...
 return <main className="grammar-course" dir="rtl">
  <header className="grammar-course-header"><Link href="/grammar"><ChevronRight/><span><b>قاعة القواعد</b><small>SALLE DE GRAMMAIRE</small></span></Link><div><span>A1</span><h1>LES FONDATIONS</h1><h2>تأسيس القواعد الفرنسية</h2><p>12 درسًا مرتبًا من بناء الجملة إلى التقديم والوصف.</p></div></header>
  <section className="grammar-course-layout">
   ...
   {active&&<article className="grammar-lesson" ref={detailRef}><div className="grammar-lesson-top"><span>LEÇON {String(active.number).padStart(2,"0")} · {active.category}</span><button onClick={()=>void speakFrench(active.titleFr,{rate:.76})}><Volume2/> نطق العنوان</button></div><h1 dir="ltr">{active.titleFr}</h1><h2>{active.titleAr}</h2><p className="grammar-lesson-summary">{active.summary}</p>
    ...
    <section className="grammar-examples"><header><Sparkles/><div><b>أمثلة محللة</b><small>اضغط على الصوت لسماع الجملة الفرنسية</small></div></header>{active.examples.map((example,index)=><div className="grammar-example" key={example.fr}><button onClick={()=>void speakFrench(example.fr,{rate:.72})} aria-label={`نطق الجملة ${example.fr}`}><Volume2/></button><div><p dir="ltr">{example.parts.map((part,partIndex)=><span key={`${part.text}-${partIndex}`} className={`role-${part.role}`}>{part.text}{partIndex<example.parts.length-1?" ":""}</span>)}</p><small>{example.ar}</small></div><i>{index+1}</i></div>)}<div className="grammar-role-key"><span className="role-subject">الفاعل</span><span className="role-verb">الفعل</span><span className="role-object">المكمل</span><span className="role-marker">الأداة</span><span className="role-adjective">الصفة</span></div></section>
    <section className="grammar-compare"><div className="correct"><Check/><span><b>صحيح</b><p dir="ltr">{active.correct}</p><button onClick={()=>void speakFrench(active.correct,{rate:.72})}><Volume2/></button></span></div><div className="incorrect"><X/><span><b>غير صحيح</b><p dir="ltr">{active.incorrect}</p></span></div></section>
    <section className="grammar-note"><AlertTriangle/><div><b>انتبه</b><p>{active.note}</p></div></section>
    <footer className="grammar-sources"><div><b>المراجع المعتمدة لهذا المنهج</b><small>تُراجع القواعد والأمثلة وفق الفرنسية المعيارية.</small></div><a href="https://www.academie-francaise.fr/questions-de-langue" target="_blank" rel="noreferrer">Académie française <ExternalLink/></a><a href="https://eduscol.education.fr/document/45262/download?attachment=" target="_blank" rel="noreferrer">Éduscol — Grammaire du français <ExternalLink/></a></footer>
   </article>}
  </section>
 </main>
}
