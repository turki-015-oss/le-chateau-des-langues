...
import {a1Lessons,a2Lessons,type GrammarLesson} from "../grammarData";
import "../grammar.css";
...
export default function GrammarLevelClient({level}:{level:string}){
 const lessons=level==="a1"?a1Lessons:level==="a2"?a2Lessons:[];const [query,setQuery]=useState("");const [activeId,setActiveId]=useState(lessons[0]?.id??"");const detailRef=useRef<HTMLElement>(null);
 ...
 return <main className="grammar-course" dir="rtl">
  <header className="grammar-course-header"><Link href="/grammar"><ChevronRight/><span><b>قاعة القواعد</b><small>SALLE DE GRAMMAIRE</small></span></Link><div><span>{level.toUpperCase()}</span><h1>{level==="a1"?"LES FONDATIONS":"STRUCTURES ESSENTIELLES"}</h1><h2>{level==="a1"?"المنهج الكامل لقواعد A1":"المنهج الكامل لقواعد A2"}</h2><p>{lessons.length} درسًا متدرجًا، بأمثلة فرنسية وترجمة عربية سياقية مدققة.</p></div></header>
