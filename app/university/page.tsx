...
export default function UniversityPage({initialLevelId,initialModuleId,levelPage=false,lessonPage=false}:UniversityPageProps={}){
 ...
 const writingMinimum=isEnhancedA1Lesson?8:60;
 const writingMaximum=isEnhancedA1Lesson?12:80;
    ...
    {lessonStage==="practice"&&<section className="university-practice-stage">
     ...
     {isEnhancedLesson&&<section className="a2-interactive-workshop">
      ...
      {revisionWorkshopPanel==="dictation"&&<article className="a2-dictation-panel">
       <div className="a2-workshop-progress"><span>{isEnhancedA1Lesson?"الكلمة":"الجملة"} {revisionDictationIndex+1} من {activeA2Dictation.length}</span><i><b style={{width:`${(revisionDictationIndex+1)/activeA2Dictation.length*100}%`}}/></i></div>
       <h4>استمع ثم اكتب {isEnhancedA1Lesson?"الكلمة":"الجملة"} الفرنسية</h4><p>يمكنك إعادة الصوت، ولا تظهر الإجابة المكتوبة إلا بعد التحقق.</p>
       <button className="a2-workshop-audio" onClick={()=>void speakFrench(revisionDictationItem.speech,{rate:isEnhancedA1Lesson?.64:.7})}><Volume2/> استمع إلى {isEnhancedA1Lesson?"الكلمة":"الجملة"}</button>
       <input dir="ltr" value={revisionDictationText} onChange={event=>{setRevisionDictationText(event.target.value);setRevisionDictationChecked(false)}} placeholder={isEnhancedA1Lesson?"Écrivez le mot ici…":"Écrivez la phrase ici…"} aria-label={`اكتب ${isEnhancedA1Lesson?"الكلمة":"الجملة"} الفرنسية التي سمعتها`}/>
       <div className="a2-workshop-actions"><button onClick={()=>setRevisionDictationChecked(true)} disabled={!revisionDictationText.trim()}><CheckCircle2/> تحقق</button>{revisionDictationIndex<activeA2Dictation.length-1&&<button className="secondary" onClick={()=>{setRevisionDictationIndex(index=>index+1);setRevisionDictationText("");setRevisionDictationChecked(false)}}>{isEnhancedA1Lesson?"الكلمة":"الجملة"} التالية <ChevronLeft/></button>}</div>
