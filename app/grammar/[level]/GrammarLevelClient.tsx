...
import {AlertTriangle,ArrowLeft,ArrowDown,BookOpen,Check,ChevronRight,Search,Sparkles,Volume2,X} from "lucide-react";
import {speakFrench} from "@/lib/frenchSpeech";
import {a1Lessons,a2Lessons,b1Lessons,b2Lessons,c1Lessons,c2Lessons,type GrammarLesson} from "../grammarData";
import "../grammar.css";
...
export default function GrammarLevelClient({level}:{level:string}){
 const lessons=level==="a1"?a1Lessons:level==="a2"?a2Lessons:level==="b1"?b1Lessons:level==="b2"?b2Lessons:level==="c1"?c1Lessons:level==="c2"?c2Lessons:[];const [query,setQuery]=useState("");const [activeId,setActiveId]=useState(lessons[0]?.id??"");const detailRef=useRef<HTMLElement>(null);
 ...
 const headings:Record<string,[string,string]>={a1:["LES FONDATIONS","المنهج الكامل لقواعد A1"],a2:["STRUCTURES ESSENTIELLES","المنهج الكامل لقواعد A2"],b1:["GRAMMAIRE INTERMÉDIAIRE","المنهج الكامل لقواعد B1"],b2:["PRÉCISION ET NUANCE","المنهج الكامل لقواعد B2"],c1:["MAÎTRISE ET DISCOURS","المنهج الكامل لقواعد C1"],c2:["MAÎTRISE ABSOLUE","المنهج الكامل لقواعد C2"]};
 return <main className="grammar-course" dir="rtl">
  ...
  <section className="grammar-course-layout">
   ...
   {active&&<article className="grammar-lesson" ref={detailRef}><div className="grammar-lesson-top"><span>LEÇON {String(active.number).padStart(2,"0")} · {active.category}</span><button onClick={()=>void speakFrench(active.titleFr,{rate:.76})}><Volume2/> نطق العنوان</button></div><h1 dir="ltr">{active.titleFr}</h1><h2>{active.titleAr}</h2><p className="grammar-lesson-summary">{active.summary}</p>
    ...
    {active.rewrite&&<section className="grammar-rewrite-lab"><header><Sparkles/><span><b>مختبر الصياغة</b><small>ATELIER DE REFORMULATION · {active.rewrite.register}</small></span></header><div className="grammar-rewrite-flow"><article><small>الصياغة الأولية القابلة للمراجعة</small><p dir="ltr">{active.rewrite.sourceFr}</p><em>{active.rewrite.sourceAr}</em></article><ArrowDown/><article className="target"><small>الصياغة المتقنة</small><p dir="ltr">{active.rewrite.targetFr}</p><em>{active.rewrite.targetAr}</em><button onClick={()=>void speakFrench(active.rewrite!.targetFr,{rate:.7})}><Volume2/> نطق الصياغة</button></article></div><p className="grammar-rewrite-explanation">{active.rewrite.explanation}</p></section>}
