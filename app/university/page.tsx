...
}function A1StudyPresentationCarousel(){
 const [presentationIndex,setPresentationIndex]=useState(0);
 const card=A1_STUDY_PRESENTATION_CARDS[presentationIndex];
 const play=()=>void speakFrench(card.fr,{rate:.72});
 const selectCard=(nextIndex:number)=>{const nextCard=A1_STUDY_PRESENTATION_CARDS[nextIndex];if(!nextCard||nextIndex===presentationIndex)return;setPresentationIndex(nextIndex);void speakFrench(nextCard.fr,{rate:.72});};
 return <div className="a1-study-presentation-carousel">
  ...
  <nav className="a1-time-greeting-navigation a1-study-presentation-navigation" dir="ltr" aria-label="التنقل بين جمل تقديم الدراسة"><button type="button" onClick={()=>selectCard(presentationIndex-1)} disabled={presentationIndex===0} aria-label="الجملة السابقة"><ChevronLeft/></button><span>{presentationIndex+1} / {A1_STUDY_PRESENTATION_CARDS.length}</span><button type="button" onClick={()=>selectCard(presentationIndex+1)} disabled={presentationIndex===A1_STUDY_PRESENTATION_CARDS.length-1} aria-label="الجملة التالية"><ChevronRight/></button></nav>
 </div>;
